...
for (n in c(50,200,2000,5000)){
  ...
  asia_result <- hc(asia_data,restart = 200)
  asia_g <- as.graphAM(asia_result)
  adjm <- attr(asia_g,'adjMat')
  row.names(adjm) <- colnames(adjm)
  write.csv (adjm,file = sprintf("experiment/result/asia-%d-hc.csv",n))
  # alarm
  alarm_result <- hc(alarm_data,restart = 200)
  alarm_g <- as.graphAM(alarm_result)
  adjm <- attr(alarm_g,'adjMat')
  row.names(adjm) <- colnames(adjm)
  write.csv (adjm,file = sprintf("experiment/result/alarm-%d-hc.csv",n))
  # insurance
  insurance_result <- hc(insurance_data,restart = 200)
  insurance_g <- as.graphAM(insurance_result)
  adjm <- attr(insurance_g,'adjMat')
  row.names(adjm) <- colnames(adjm)
  write.csv (adjm,file = sprintf("experiment/result/insurance-%d-hc.csv",n))

  # hailfinder
  hailfinder_result <- hc(hailfinder_data,restart = 200)
  hailfinder_g <- as.graphAM(hailfinder_result)
  adjm <- attr(hailfinder_g,'adjMat')
  row.names(adjm) <- colnames(adjm)
  write.csv (adjm,file = sprintf("experiment/result/hailfinder-%d-hc.csv",n))
