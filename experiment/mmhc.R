...
for (n in c(50,200,2000,5000)){
  ...
  asia_result <- mmhc(asia_data)
  asia_g <- as.graphAM(asia_result)
  adjm <- attr(asia_g,'adjMat')
  row.names(adjm) <- colnames(adjm)
  write.csv (adjm,file = sprintf("experiment/result/asia-%d-mmhc.csv",n))
  # alarm
  alarm_result <- mmhc(alarm_data)
  alarm_g <- as.graphAM(alarm_result)
  adjm <- attr(alarm_g,'adjMat')
  row.names(adjm) <- colnames(adjm)
  write.csv (adjm,file = sprintf("experiment/result/alarm-%d-mmhc.csv",n))
  # insurance
  insurance_result <- mmhc(insurance_data)
  insurance_g <- as.graphAM(insurance_result)
  adjm <- attr(insurance_g,'adjMat')
  row.names(adjm) <- colnames(adjm)
  write.csv (adjm,file = sprintf("experiment/result/insurance-%d-mmhc.csv",n))

  # hailfinder
  hailfinder_result <- mmhc(hailfinder_data)
  hailfinder_g <- as.graphAM(hailfinder_result)
  adjm <- attr(hailfinder_g,'adjMat')
  row.names(adjm) <- colnames(adjm)
  write.csv (adjm,file = sprintf("experiment/result/hailfinder-%d-mmhc.csv",n))
